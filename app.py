...
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get('DATABASE_URL') or "postgresql://laurentahari@localhost/taharilHW4db" # TODO 364: You should edit this to correspond to the database name YOURUNIQNAMEHW4db and create the database of that name (with whatever your uniqname is; for example, my database would be jczettaHW4db). You may also need to edit the database URL further if your computer requires a password for you to run this.
...
tags = db.Table('tags', db.Column('gif_id', db.Integer, db.ForeignKey('Gif.id')), db.Column('term', db.String(32), db.ForeignKey('Search_Term.term')))
...
user_collection = db.Table('collection', db.Column('gif_id', db.Integer, db.ForeignKey('Gif.id')), db.Column('collection_id', db.Integer, db.ForeignKey('GifCollection.id')))
...
class SearchTerm(db.Model):
    ...
    gifs = db.relationship('Gif', secondary = tags, backref = db.backref('Search_Term', lazy = 'dynamic'), lazy = 'dynamic')
...
def get_gifs_from_giphy(search_string):
    ...
    giflist = []
    dic_params = {'api_key':api_key, 'q':search_string, 'limit': 5}
    response = requests.get(baseurl, params = dic_params)
    text = response.text
    giflist = json.loads(text)['data']
    return giflist
...
def get_or_create_gif(db_session, title, url):
    ...
    gifinstance = db_session.query(Gif).filter_by(title = title).first()
    if gifinstance:
        return gifinstance
    else:
        gifinstance = Gif(title = title, embedURL = url)
        db_session.add(gifinstance)
        db_session.commit()
    return gifinstance
...
def get_or_create_search_term(db_session, term):
    ...
    searchterminstance = db_session.query(SearchTerm).filter_by(term = term).first()
    if searchterminstance:
        ...
        mysearch = get_gifs_from_giphy(term)
        for gif in mysearch:
            gifinstance = get_or_create_gif(db_session, title = gif['title'], url = gif['embed_url'])
            searchterminstance.gifs.append(gifinstance)
        db_session.add(searchterminstance)
        db_session.commit()
        return searchterminstance
...
def get_or_create_collection(db_session, name, current_user, gif_list):
    ...
    my_gifcollection = PersonalGifCollection.query.filter_by(name = name, user_id = current_user.id).first()
    if my_gifcollection:
        return my_gifcollection
    else:
        my_gifcollection = PersonalGifCollection(name = name, user_id = current_user.id, gifs = [])
        for gifinstance in gif_list:
            my_gifcollection.gifs.append(gifinstance)
        db_session.add(my_gifcollection)
        db_session.commit()
        return my_gifcollection
...
def search_terms():
    ...
    searched_terms = SearchTerm.query.all()
    return render_template('search_terms.html', all_terms = searched_terms)
...
def create_collection():
    ...
    if request.method == 'POST':
        chosengif = form.gif_picks.data
        giffy = [get_gif_by_id(int(id)) for id in chosengif]
        get_or_create_collection(db.session, name = form.name.data, current_user = current_user, gif_list = giffy)
        print('You have created a collection!')
...
def collections():
    ...
    specific_collection = PersonalGifCollection.query.filter_by(user_id = current_user.id).all()
    return render_template('collections.html', collections = specific_collection)
